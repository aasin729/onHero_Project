...
const Excavation = () => {
    ...
    tooltip: {
      ...
      y: {
        formatter: (value, { seriesIndex, dataPointIndex }) => {
          const keys = ["ourfrcs_roka", "ourfrcs_unmil", "et_nk", "et_chn", "idntycfmtn_kia"];
  ...
  const generateChartSeries = (chunk) => [
    { name: "아군 국군", data: chunk.map((item) => parseFloat(item.ourfrcs_roka)) },
    { name: "아군 UN군", data: chunk.map((item) => parseFloat(item.ourfrcs_unmil)) },
    { name: "적군 북한", data: chunk.map((item) => parseFloat(item.et_nk)) },
    { name: "적군 중국", data: chunk.map((item) => parseFloat(item.et_chn)) },
    { name: "신원확인 전사자", data: chunk.map((item) => parseFloat(item.idntycfmtn_kia)) },
  ];
  ...
  const formattedData = data.map((item) => {
    ...
    return {
      ...
      ourfrcs_roka: ((parseInt(item.ourfrcs_roka || 0) / total) * 100).toFixed(2),
      ourfrcs_unmil: ((parseInt(item.ourfrcs_unmil || 0) / total) * 100).toFixed(2),
      et_nk: ((parseInt(item.et_nk || 0) / total) * 100).toFixed(2),
      et_chn: ((parseInt(item.et_chn || 0) / total) * 100).toFixed(2),
      idntycfmtn_kia: ((parseInt(item.idntycfmtn_kia || 0) / total) * 100).toFixed(2),
    };
  });
